...
Se optó por una arquitectura modular basada en componentes especializados, donde cada módulo desempeña una función concreta dentro del flujo de procesamiento de eventos. Esta organización permite mantener una clara separación de responsabilidades entre la ingesta, el almacenamiento, la correlación, la detección de anomalías y la presentación de la información. Como resultado, se obtiene una solución flexible, fácilmente mantenible y preparada para incorporar nuevas funcionalidades o fuentes de información en futuras versiones.
...
Con el fin de representar los diferentes elementos de seguridad presentes en el entorno monitorizado, se tomaron como referencia diversos dispositivos habitualmente utilizados en instalaciones de alta seguridad. Aunque el sistema desarrollado opera sobre eventos simulados, estos dispositivos sirvieron como base para definir el comportamiento y la información generada por cada tipo de sensor integrado en el SIEM.

Las figuras siguientes muestran algunos de los dispositivos tomados como referencia para modelar los distintos elementos de monitorización presentes en el entorno simulado.

\imagen{img/camara}
{Cámara de vigilancia utilizada como referencia para la monitorización perimetral.}
{0.55}

\imagen{img/sensor_puerta}
{Sensor magnético de apertura de puerta utilizado como referencia para el control de accesos.}
{0.45}

\imagen{img/sensor_movimiento}
{Sensor volumétrico de movimiento utilizado para la detección de presencia en el interior de los depósitos.}
{0.45}

\imagen{img/sensor_temperatura}
{Sensor ambiental de temperatura utilizado para la supervisión de las condiciones del depósito.}
{0.45}

\imagen{img/sensor_humedad}
{Sensor ambiental de humedad utilizado para la supervisión de las condiciones del depósito.}
{0.45}
...
Las alertas generadas por el sistema tienen como finalidad apoyar la toma de decisiones del personal responsable de la instalación. En consecuencia, cualquier actuación correctora derivada de una incidencia detectada requiere la intervención presencial de los operadores o responsables de seguridad, no contemplándose mecanismos de respuesta automática sobre la infraestructura monitorizada.

Asimismo, el proyecto no contempla mecanismos específicos de cifrado o autenticación en la transmisión de información procedente de los sensores. Dado que se trata de un entorno simulado, se ha asumido que los dispositivos integrados operan dentro de una infraestructura controlada y aislada. En una implantación real sería recomendable incorporar protocolos de comunicación seguros y mecanismos de autenticación que garanticen la integridad y confidencialidad de la información transmitida.
...
La Figura \ref{fig:img/logs} muestra un ejemplo de registros almacenados en los archivos de log utilizados durante las pruebas. Puede observarse que, independientemente del tipo de dispositivo que genera el evento (control de acceso, sensor magnético, sensor de movimiento, sensor ambiental o sistema eléctrico), todos los registros mantienen una estructura homogénea. Esta normalización facilita el procesamiento automático de la información por parte del módulo de ingesta y simplifica las tareas de almacenamiento, correlación y análisis realizadas posteriormente por el sistema SIEM.

\imagen{img/logs}
{Ejemplo de eventos generados por diferentes dispositivos utilizando un formato normalizado común.}
{0.90}
...
Aunque inicialmente se valoró la utilización de técnicas de aprendizaje automático (Machine Learning), finalmente se optó por una solución basada en reglas. Esta decisión se tomó con el objetivo de disponer de un sistema ligero, fácilmente interpretable y sencillo de mantener. Además, dado que las incidencias detectadas requieren la intervención y valoración del personal responsable de la instalación, se consideró más adecuado utilizar un enfoque basado en reglas claramente definidas que facilitara la comprensión y justificación de las alertas generadas.
...
Durante el diseño se optó por una estructura sencilla compuesta por una tabla destinada al almacenamiento de eventos y una segunda tabla dedicada al registro de alertas generadas por el sistema. Esta aproximación permite mantener una organización clara de la información y facilita futuras ampliaciones del sistema.
...
La documentación de la API se genera automáticamente mediante Swagger \cite{swagger}. Además de mostrar los endpoints disponibles, permite ejecutar consultas y visualizar las respuestas obtenidas directamente desde el navegador.
...
Como complemento al sistema de monitorización, se implementó un mecanismo de notificaciones SMS para alertas críticas mediante el servicio Twilio. Cuando una alerta alcanza el nivel de riesgo configurado, el sistema puede generar y enviar automáticamente un mensaje SMS al responsable designado, permitiendo una notificación inmediata de incidencias relevantes.

\imagen{img/dashboard_3sms}
{Configuración del envío de notificaciones SMS para alertas críticas.}
{0.90}

\imagen{img/recepcion_sms}
{Recepción de una alerta crítica mediante SMS en un dispositivo móvil.}
{0.60}

\imagen{img/sms_consola}
{Registro del envío de una notificación SMS durante la ejecución del sistema.}
{0.80}

La configuración del servicio permite habilitar o deshabilitar el envío de mensajes y registrar las notificaciones realizadas, facilitando su integración dentro del flujo de monitorización y respuesta ante incidentes.
...
Dado que el objetivo principal del proyecto se centra en la monitorización y gestión de eventos de seguridad, no se han implementado mecanismos avanzados de autenticación de usuarios, cifrado de comunicaciones mediante HTTPS ni control de accesos basado en credenciales. Esta decisión se justifica porque el sistema ha sido concebido para operar dentro de una red local restringida ubicada en el recinto de control y supervisión de la instalación, sin acceso desde otras dependencias ni desde redes externas. Asimismo, el entorno simulado utilizado durante el desarrollo asume que los dispositivos y sensores integrados forman parte de una infraestructura controlada y aislada.
