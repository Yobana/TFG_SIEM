...
Splunk \cite{splunk} es una de las plataformas SIEM más utilizadas a nivel empresarial. Permite la recopilación y análisis de grandes volúmenes de datos procedentes de múltiples fuentes, proporcionando capacidades avanzadas de búsqueda, generación de informes y visualización.
...
IBM QRadar \cite{qradar} es una plataforma SIEM orientada principalmente a grandes organizaciones. Dispone de mecanismos avanzados de correlación, análisis de comportamiento y detección de amenazas.
...
Wazuh \cite{wazuh} es una plataforma de código abierto ampliamente utilizada en entornos de monitorización y ciberseguridad. Ofrece funcionalidades relacionadas con la recopilación de eventos, detección de intrusiones, monitorización de integridad y generación de alertas.
...
Actualmente constituye una de las plataformas SIEM de código abierto más utilizadas tanto en entornos académicos como profesionales \cite{wazuh}.

\section{Elastic Stack}

Elastic Stack \cite{elastic}, también conocido como ELK Stack (Elasticsearch, Logstash y Kibana), es una plataforma muy utilizada para la gestión y visualización de grandes volúmenes de información.
...
Elastic Stack es ampliamente utilizado como base para soluciones de monitorización, análisis de eventos y construcción de plataformas SIEM personalizadas \cite{elastic}.
...
Entre ellos destaca el trabajo "Diseño e Implementación de una Plataforma de Detección de Amenazas de Red", desarrollado en la Universidad de Valladolid \cite{valladolid}, donde se plantea una solución orientada a la detección de amenazas mediante herramientas de monitorización y análisis de eventos de seguridad.

También resulta especialmente relevante el trabajo "Implementación de un SIEM Open Source para la Monitorización de Seguridad: Análisis de Logs con Wazuh", desarrollado en la Universitat Politècnica de València \cite{upvsiem}. En este proyecto se implementa una plataforma SIEM basada en Wazuh para la recopilación y análisis de eventos procedentes de distintos sistemas.

Asimismo, se revisó un proyecto de la Universidad de Las Palmas de Gran Canaria centrado en el despliegue de un clúster Wazuh para la monitorización de eventos de seguridad en entornos distribuidos \cite{ulpgcwazuh}.

Finalmente, se estudiaron trabajos relacionados con la aplicación de técnicas de Inteligencia Artificial en Centros de Operaciones de Seguridad (SOC), donde las plataformas SIEM se utilizan como elemento central para la recopilación y análisis de información de seguridad \cite{ia_soc}.
...
Aunque el entorno desarrollado en este Trabajo Fin de Grado corresponde a una simulación académica, el escenario seleccionado se inspira en un polvorín militar, una instalación que, debido a la naturaleza de los materiales almacenados, requiere elevados niveles de protección física y supervisión continua.
...
\begin{table}[htbp]
	...
	\begin{tabular}{|l|c|c|c|c|c|}
		\hline
		\textbf{Solución} & \textbf{Coste} & \textbf{Despliegue} & \textbf{Recursos} & \textbf{Escalabilidad} & \textbf{ML/IA} \\
		\hline
		Splunk & Alto & Media & Alto & Muy alta & Sí \\
		IBM QRadar & Alto & Complejo & Alto & Muy alta & Sí \\
		Wazuh & Bajo & Media & Medio & Alta & Parcial \\
		Elastic Stack & Bajo & Media & Medio & Alta & Parcial \\
		TFG-SIEM & Muy bajo & Sencillo & Muy bajo & Limitada & No \\
		\hline
	\end{tabular}
...
La comparación realizada pone de manifiesto que la solución desarrollada no pretende competir funcionalmente con plataformas SIEM comerciales o empresariales como Splunk o QRadar. Sin embargo, presenta ventajas significativas en términos de simplicidad de despliegue, bajo consumo de recursos, facilidad de comprensión y reducido coste de implantación. Estas características la convierten en una alternativa adecuada para entornos de pequeño tamaño, fines formativos o escenarios donde no se requiera la complejidad de las soluciones profesionales.

La principal ventaja de la solución desarrollada reside en su sencillez de despliegue y reducido consumo de recursos, pudiendo ejecutarse sobre hardware de bajo coste como una Raspberry Pi. Por el contrario, las plataformas profesionales analizadas ofrecen una mayor capacidad de escalado, integración con fuentes heterogéneas y funcionalidades avanzadas de análisis que exceden el alcance del presente proyecto.
